export const apiServer = {
     serverUrls: {
        categorias: 'http://localhost/assets/data/categorias.json',
        ofertas: 'http://localhost/assets/data/ofertas.json',
        especialidades: 'http://localhost/assets/data/especialidades.json',
        entrements: 'http://localhost/assets/data/entremets.json',
        tortas: 'http://localhost/assets/data/tortas.json',
        entradas: 'http://localhost/assets/data/entradas.json'
     },
     siteName: 'Delirios'
}

